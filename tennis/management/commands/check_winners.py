...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ts = datetime.date(2015, 1, 4)
        n_errors = 0
        matches = Match.objects.filter().exclude(
            pk__in=(42921,)
        ).order_by('start_ts')
        im = 0
        for m in matches:
            ...
            if match_betting_has_result:
                ...
                if has_winner:
                    ...
                    if winner != m.winner:
                        n_errors += 1
                        print "###### WINNER:", m.pk, " :: result", winner.getName(), " vs ", m.winner.getName() if m.winner else 'None', " errors: ", n_errors
                        m.winner = winner
                        m.save()
                    else:
                        print "OK: ", winner.getName(), " :: ", m.winner.getName()
                    continue
                else:
                    mb_void = True
            ...
            print "Void?: ", mb_live_void
            if mb_live_void:
                if m.winner is not None:
                    print "###### Winner1 not void!", m.pk, m.winner.getName()
                if m.void is not True:
                    print "###### Winner2 not void!", m.pk, m.void

                if not m.winner and m.void:
                    print "Void!, ok!"
                else:
                    n_errors += 1
                    print "###### Winner not void!", m.pk, " errors: ", n_errors
                    print m.winner
                    print m.void
                    m.setVoid()
            continue
